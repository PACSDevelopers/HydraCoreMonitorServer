...
abstract class :xhp implements XHPChild {
  ...
  abstract public function appendChild(mixed $child): string;
   ...
   * though.
   */
  public bool $ENABLE_VALIDATION = true;
  ...
  final protected static function renderChild($child) {
    if ($child instanceof :xhp) {
      ...
      $string = htmlspecialchars($child);

      $entities = array_diff(get_html_translation_table(HTML_ENTITIES),get_html_translation_table(HTML_SPECIALCHARS));

      // Fix everything leftover
      foreach($entities as $encodedEntity) {
        $string = str_replace('&amp;' . substr($encodedEntity, 1), $encodedEntity, $string);
      }
...
abstract class :x:composable-element extends :x:base {
  ...
  final protected function __flushRenderedRootElement(): :x:primitive {
    ...
    if ($composed instanceof :x:primitive) {
      $composed->transferContext($that->context);
    } else if ($this->ENABLE_VALIDATION) {
  ...
  final protected function validateChildren(): void {
    ...
    if (!$ret || $ii < count($this->children)) {
	  if (isset($this->children[$ii])) {
	    if($this->children[$ii] instanceof HTML) {
	        return;
	    } else if(method_exists($this->children[$ii], 'render')) {
	        return;
	    }
	  }
...
class XHPInvalidChildrenException extends XHPException {
  public function __construct($that, int $index) {
    parent::__construct(
