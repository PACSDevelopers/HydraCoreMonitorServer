...
class ProcessChartDataAjax extends \HC\Ajax {
    ...
    public function init($GET = [], $POST = []) {
        ...
        if(!$result = $cache->select('\HCPublic\Ajax\Databases\Charts\ProcessChartDataAjax' . $POST['scale'])) {
            ...
            $db = new \HC\DB();
            $result = $db->query('SELECT `DHO`.`percent`, `DHO`.`responseTime`, UNIX_TIMESTAMP(`DHO`.`dateCreated`) as `dateCreated` FROM `database_history_overview` `DHO` WHERE `DHO`.`dateCreated` > ?;', [$currentDate24]);
            ...
            $cache->insert('\HCPublic\Ajax\Databases\Charts\ProcessChartDataAjax' . $POST['scale'], $result, 300);
        }
