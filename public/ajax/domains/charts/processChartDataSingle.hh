...
class ProcessChartDataSingleAjax extends \HC\Ajax {
    ...
    public function init($GET = [], $POST = []) {
        ...
        if(!$result = $cache->select('\HCPublic\Ajax\Domains\Charts\ProcessChartDataSingleAjax' . $POST['scale'] . $POST['domainID'])) {
            ...
            $db = new \HC\DB();
            $result = $db->query('SELECT `DH`.`status`, `DH`.`responseTime`, UNIX_TIMESTAMP(`DH`.`dateCreated`) as `dateCreated` FROM `domain_history` `DH` WHERE `DH`.`domainID` = ? AND `DH`.`dateCreated` > ?;', [$POST['domainID'], $currentDate24]);
