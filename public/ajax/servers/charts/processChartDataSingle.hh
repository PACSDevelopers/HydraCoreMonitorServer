...
class ProcessChartDataSingleAjax extends \HC\Ajax {
    ...
    public function init($GET = [], $POST = []) {
        ...
        if(!$result = $cache->select('\HCPublic\Ajax\Servers\Charts\ProcessChartDataSingleAjax' . $POST['scale'] . $POST['serverID'])) {
            ...
            $db = new \HC\DB();
            $result = $db->query('SELECT `SH`.`status`, `SH`.`domainID`, `SH`.`responseTime`, `SH`.`cpu`, `SH`.`mem`, `SH`.`iow`, `SH`.`ds`, `SH`.`ds`, `SH`.`net`, `SH`.`rpm`, `SH`.`tps`, `SH`.`qpm`, `SH`.`avgTimeCpuBound`, `SH`.`avgRespTime`, UNIX_TIMESTAMP(`SH`.`dateCreated`) as `dateCreated` FROM `server_history` `SH` WHERE `SH`.`serverID` = ? AND `SH`.`dateCreated` > ?;', [$POST['serverID'], $currentDate24]);
